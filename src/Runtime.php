...
class Runtime extends IronCore
{
    ...
    public static function getArgs($assoc = true)
    {
        ...
        foreach ($argv as $k => $v) {
            ...
            if ($v == '-payload') {
                $args['payload_file'] = $argv[$k + 1];
            }

            if ($v == '-config') {
                $args['config_file'] = $argv[$k + 1];
            }

        }

        if (getenv('TASK_ID')) {
            $args['task_id'] = getenv('TASK_ID');
        }

        if (getenv('TASK_DIR')) {
            $args['dir'] = getenv('TASK_DIR');
        }

        if (getenv('PAYLOAD_FILE')) {
            $args['payload_file'] = getenv('PAYLOAD_FILE');
        }

        if (getenv('CONFIG_FILE')) {
            $args['config_file'] = getenv('CONFIG_FILE');
        }
